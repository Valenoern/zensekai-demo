...
_.extend(Passage.prototype,
{
	...
	**/
	...
	render: function()
	{
		console.info('showing passage');
		
		// SHUES: add an even more abbreviated syntax to do single func calls.
		// this must be addressed before proper underscore templating happens in order to work correctly.
		// also it's exactly the same as md's blockquote syntax so you can't use a single function call of the form "> func();" as a blockquote.
		
		// previously I used this regex but one day it randomly started hanging the browser? so I rewrote it from scratch
		// /\>\s([a-zA-Z0-9_\.]+\(([\"\']?.+[\"\']?,?\s?)*\);\n)/g
		
		var source = _.unescape(this.source);
		//onsole.log('before: ', source);
		source = source.replace(/\>\s([A-Za-z0-9_\.]+\(([\'\"].*[\'\"],?\s?)*\);\n?)/g, '<% $1 %>');
		//onsole.log('after: ', source);
		
			
		// we have to temporarily disable window.print, as it
		// interferes with Underscore's template print function

		var result = _.template(source, { s: window.story.state, $: this._readyFunc });
		...
		// SHUES: support [~[]] links as a convenient hack to disable arrows in the editor
