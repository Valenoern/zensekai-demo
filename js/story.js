	...
	start: function()
	{
		...
		**/
		...
		if (window.location.hash === '' || ! this.restore(window.location.hash.replace('#', '')))
		{
			...
			// SHUES: wrap rendering the passage in a check that shues has loaded.
			
			console.info('set function for when shues loaded');
			
			$(window).on('shues_loaded', (function() {
				this.show(this.startPassage);
				this.atCheckpoint = true;
				
				console.info('func for when shues loaded ran');
			}).bind(this));
			
			console.info('ok to load shues');
			$.event.trigger('shues_load');
		}
	},
